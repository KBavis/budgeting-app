...
public interface TransactionRepository extends JpaRepository<Transaction, String> {
    ...
    @Query("SELECT t FROM Transaction t WHERE t.account.accountId IN :accountIds AND t.is_deleted IS FALSE AND (t.date IS NOT NULL AND EXTRACT(MONTH FROM CAST(t.date AS date)) = EXTRACT(MONTH FROM CAST(:currentDate AS date)) AND EXTRACT(YEAR FROM CAST(t.date AS date)) = EXTRACT(YEAR FROM CAST(:currentDate AS date)))")
