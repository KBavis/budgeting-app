...
def fetch_transactions(user_id, connection): 
    ...
    query = """
        SELECT category_id, amount, date_time, COALESCE(merchant_name, name) AS merchant_name, plaid_detailed_category, plaid_primary_category
        FROM transaction 
        WHERE account_id IN (SELECT account_id FROM account WHERE user_id = %s)
        AND category_id IS NOT NULL AND logo_url NOT LIKE '%%venmo%%' AND is_deleted = False
    """
